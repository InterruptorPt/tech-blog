...
description: How to get your site to respond when you're receiving payments through Web Monetization. Tutorial passo-a-passo com React e TypeScript.
...
Apesar de bastar algo como `document.monetization === 'started'` para fazer essa verificação, se estivermos a usar uma _framework_ como React, convém que a páginaa atualize sempre que o processo de pagamento inicie ou termine.
...
## Passo 1: Adicionar ouvintes aos eventos
...
1. Antes de tentarmos adicionar ouvintes aos eventos, temos sempre de verificar se existe `document.monetization` para podermos adicioná-los.
1. Devemos certificar-nos que removemos ou ouvinte do evento, quando já não estamos a usá-lo.
...
/**
 * Isto adiciona um ouvinte ao evento de monetização e devolve uma função
...
Isto vai dar jeito quando adicionarmos aos eventos com `useEffect` – podemos adicionaar um ouvinte e depois removê-lo com uma função de limpeza:
...
Inicializar o contexto com é `undefined` é um pequeno truque que uso para ter a certeza que não tento aceder ao `WebMonetizationContext` acidentalmente, fora do `Provider`. Se definires um valor padrão ao usar React Context, podes esquecer-te de encapsular a tua aplicação com o componente `Provider` sem nunca te aperceberes. Já me aconteceu ser apanhado em coisas assim algumas vezes, por isso permito que o valor esteja `undefined`, e depois crio um _hook_ que dispara um erro se o valor do contexto estiver em falta.
...
A única coisa que falta é encapsular a aaplicaação com `WebMonetizationProvider`, e está tudo pronto para que possas começar a usar `context` nos teus componentes.

## Passo 3: Reagir a eventos de monetização

Agora que a nossa aplicação estáa configurada para acompanhar eventos de monetização, provavelmente só há mais um pormenor que nos interessa: _"Está ligado ou não?"_
